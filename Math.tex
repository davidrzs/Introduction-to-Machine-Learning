\section*{Useful math}
\pseudosubsection{Probabilities}\\
...
\pseudosubsection{{Bayes Rule}}
$P(A|B) = \frac{P(B|A) \cdot P(A)}{P(B)}$\\
\pseudosubsection{P-Norm}
$||x||_p = (\sum_{i=1}^n|x_i|^p)^{\frac{1}{p}}$, $1 \leq p < \infty$\\
\pseudosubsection{Some gradients}
$\nabla_x ||x||_2^2 = 2 x$\\
$f(x) = x^T A x$; $\nabla_x f(x) = (A + A^T) x$\\
...
\pseudosubsection{Convex / Jensen's inequality}
$\text{g(x) convex} \Leftrightarrow g''(x) > 0 \Leftrightarrow x_1,x_2 \in \mathbb{R}, \lambda \in [0,1]: 
g(\lambda x_1 + (1-\lambda) x_2) \leq \lambda g(x_1) + (1-\lambda) g(x_2)$\\
\pseudosubsection{Gaussian / Normal Distribution}
$f(x) = \frac{1}{\sqrt{2\pi\sigma^2}} exp(-\frac{(x-\mu)^2}{2\sigma^2})$ and $\frac{X-\mu}{\sigma} \sim \mathcal{N}(0,1)$ and $\Phi(-x) = 1 - \Phi(x)$ and ${\Phi^{-1}(x) = -\Phi^{-1}(1-x)}$. For $X\sim N(\mu_X,\sigma^2_X)$, $Y \sim N(\mu_Y, \sigma^2_Y)$ with $Z=X+Y$ yields $Z \sim N(\mu_X+\mu_Y, \sigma^2_X + \sigma^2_Y)$.\\
\pseudosubsection{Multivariate Gaussian}
$\Sigma =$ covariance matrix, $\mu$ = mean\\
$f(x) = \frac{1}{2\pi \sqrt{|\Sigma|}} e^{- \frac{1}{2} (x-\mu)^T \Sigma^{-1} (x-\mu)}$\\
Empirical: $\hat{\Sigma} = \frac{1}{n}\sum_{i=1}^n x_i x_i^T$ (needs centered data points)\\
\pseudosubsection{Pos sem.-def mat}
