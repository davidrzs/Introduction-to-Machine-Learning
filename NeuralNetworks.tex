...
\pseudosubsection{Activation functions}\\
...
\pseudosubsection{Predict: forward propagation}
$v^{(0)} = x$; for $l = 1,...,L-1$: \\
$v^{(l)} = \varphi(z^{(l)})$, $z^{(l)} = W^{(l)}v^{(l-1)}$\\
$f = W^{(L)}v^{(L-1)}$\\
Predict $f$ for regression, $\operatorname{sign}(f)$ for class.\\
\pseudosubsection{Compute gradient: backpropagation}
Output layer: 
$\delta_j = l_j'(f_j)$,
...
\pseudosubsection{Learning with momentum}
$a \leftarrow m \cdot a + \eta_t \nabla_W l(W;y,x)$; $W_{t+1} \leftarrow W_t - a$\\
\pseudosubsection{Dimensions in Convolution} $out_{size} = \lfloor \frac{in_{size} + 2\cdot padding - dilation*(kernel_{size} -1) - 1}{stride}  +1  \rfloor $